...
@RestController
@RequestMapping("/master-admin/api/ir-menu-items")
public class IRMenuItemController extends BaseController {
	...
	public ResponseEntity<IRMenuItemBO> create(@RequestBody @Valid IRMenuItemBO irm, BindingResult result,
		...
		headers.setLocation(builder.path("/master-admin/api/ir-menu-items/{id}").buildAndExpand(updtIrm.getId()).toUri());
	...
	public IRMenuItemBO delete(@RequestBody IRMenuItemBO irm, @PathVariable Long id, @ModelAttribute("serviceRequest") ServiceRequest sreq) {
		LOGGER.debug("Request for irm delete received: irm Key {}", id);
		irm.setId(id);
		sreq.addRequestData(irm);
		ServiceResponse response = irs.delete(sreq);
		return getResultBO(response, IRMenuItemBO.class);
	}
