...
<title>OMT Webservice SMA Changing</title> 
...
$(document).ready(function(){
    ...
    $(function(){
    	var dgSmaDiff = $('#dgSmaDiff').datagrid({
    		height:$(window).innerHeight()-100,
            pageList:[50,20,100,200],
            url: '<%=request.getContextPath()%>/stat/smadifflist',
            pagination: true,
            remoteFilter: true,
            rownumbers: true
        });
    	
    	dgSmaDiff.datagrid('enableFilter', [{
            field:'date',
            type:'datebox',
            options:{
	            ...
	            onChange:function(value){
	                if (value == ''){
	                    dgSmaDiff.datagrid('removeFilterRule', 'date');
	                } else {
	                    dgSmaDiff.datagrid('addFilterRule', {
	                        field: 'date',
	                        op: 'equal',
	                        value: value
	                    });
	                }
	                dgSmaDiff.datagrid('doFilter');
	            },
              panelWidth: '240',
              editable: true
            }
            ...
            options:{
	            ...
	            onChange:function(value){
	                if (value == ''){
	                    dgSmaDiff.datagrid('removeFilterRule', 'type');
	                } else {
	                    dgSmaDiff.datagrid('addFilterRule', {
	                        field: 'type',
	                        op: 'equal',
...
<body><center>
	...
	<table id="dgSmaDiff" title="SMA Changing List" class="easyui-datagrid" style="width:95%;height:600px;"
			url="<%=request.getContextPath()%>/stat/smadifflist" idFiled="id"
