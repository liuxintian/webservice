package com.omt.webservice.security.main.restsec;
...
public final class TokenAuthenticationFilter extends GenericFilterBean {
	...
	private static final String HEADER_TOKEN_VALID = "X-Token-Valid";
		...
		throws IOException, ServletException
	{
		...
		if(!authenticated){
			httpResponse.setHeader(HEADER_TOKEN_VALID, "false");
		}
	...
	private void checkUsernameAndPassword(String username, String password, HttpServletResponse httpResponse) throws IOException {
		...
		if (tokenInfo != null) {
			...
			httpResponse.setHeader(HEADER_TOKEN_VALID, "true");
	...
	private boolean checkToken(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		if (token == null) {
			httpRequest.setAttribute(HEADER_TOKEN_VALID, "false");
			return false;
		}
